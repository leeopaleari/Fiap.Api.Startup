...
CREATE TABLE T_USUARIO (
  ...
  documento         VARCHAR2(13) UNIQUE NOT NULL
);
...
CREATE TABLE T_PROPOSTA_SEGURO (
    id                  NUMBER PRIMARY KEY,
    data_criacao        DATE NOT NULL,
    data_validade       DATE NOT NULL,
    vlr_seguro          NUMBER(10,2) NOT NULL,
    observacoes         VARCHAR2(255),
    status              VARCHAR2(20),
    t_usuario_id        NUMBER NOT NULL,
    t_veiculo_id        NUMBER NOT NULL,
    CONSTRAINT fk_proposta_cliente FOREIGN KEY (t_usuario_id) REFERENCES T_USUARIO(id),
    CONSTRAINT fk_proposta_veiculo FOREIGN KEY (t_veiculo_id) REFERENCES T_VEICULO(id)
);

CREATE SEQUENCE PROPOSTA_ID_SEQ;

CREATE OR REPLACE TRIGGER TR_SEQ_PROPOSTA_SEGURO BEFORE INSERT ON T_PROPOSTA_SEGURO FOR EACH ROW  
BEGIN 
	SELECT PROPOSTA_ID_SEQ.NEXTVAL INTO :new.ID FROM dual; 
END; 

--DROP TRIGGER TR_SEQ_PROPOSTA_SEGURO; 
--DROP SEQUENCE PROPOSTA_ID_SEQ; 
--DROP TABLE T_PROPOSTA_SEGURO CASCADE CONSTRAINTS;
